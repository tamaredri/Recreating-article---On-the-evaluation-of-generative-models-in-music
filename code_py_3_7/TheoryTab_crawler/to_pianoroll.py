...
def proc_midi_to_pianoroll(filename, beats_in_measure):
    ...
    # Get the beat resolution from the first track (assuming all tracks have the same resolution)
    beat_resolution = multitrack.tracks[0].pianoroll.shape[0] // (multitrack.get_max_length() // beats_in_measure)

    # Create a downbeat array if it doesn't exist
    if not hasattr(multitrack, 'downbeat'):
        multitrack.downbeat = np.zeros((multitrack.get_max_length(),), dtype=bool)

    # Set downbeat based on the beat resolution and beats in a measure
    multitrack.downbeat[0::beat_resolution * beats_in_measure] = True
