...
root_dir = '../hooktheory-data/datasets_new'
root_xml = '../hooktheory-data/xml_new'
root_event = '../hooktheory-data/datasets_new/event'
root_pianoroll = '../hooktheory-data/datasets_new/pianoroll'
log_dir = '../hooktheory-data/analysis_new/log'
...
def proc(xml_list, queue, index=0):
    ...
    for xidx in range(num_xml):  # num_xml
        ...
        # tmp = path.split('\\')
        url = 'https://www.hooktheory.com/theorytab/view/' + path + '/' + fn
        try:
            ...
            path_event = os.path.join(root_event, path, fn)
            path_pianoroll = os.path.join(root_pianoroll, path, fn)
